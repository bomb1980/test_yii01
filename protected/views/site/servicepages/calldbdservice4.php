...
<body>
	...
	// $model = BranchTmpTb::getDatas( $bgdatep );
		...
		<table id="wpddt1" class="display row-border responsive nowrap" style="width:100%; height:auto; color:#003;">
			...
			<tbody>
				...
				foreach ($model as $rows) {
					...
					//** send email *******
					// $qemail = BranchTmpTb::model()->findByAttributes(array('crop_id' => 8771));


					// arr($qemail);
					$corpemail = $rows['email'];
					$statemail = $rows['brch_status'];
					$brnremark = $rows['brch_remark']; //ดึงขอมูลสาขาเพิ่มเพื่อส่งให้ etp
