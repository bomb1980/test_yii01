...
<head>
<meta charset="utf-8" />
<title>Get Textfile for sapiens2</title>
<script>
	...
	function writedatatotextfile(tffs_id,rowno,tffs_name){
		//alert('test , ' + rowno + ',' + tffs_name);
		var action = 'writedatatotextfile';
		dilg1 = new BootstrapDialog({
			...
			message: $('<div class="thfont5"></div>').load(<?php echo "'" . Yii::app()->createAbsoluteUrl('site/writedatatotextfile') ."'" ; ?>, { action: action, tffs_id: tffs_id, tffs_name: tffs_name  }),
			draggable: true,
			closable: true,	
	...
	function deletefileoldwpd(tffs_id,rowno,tffs_name){
		var data = "";
		...
		BootstrapDialog.confirm({
			...
			callback: function(result) {
				if(result){
					$.ajax({
						type: "POST",
						url: "<?php echo Yii::app()->createAbsoluteUrl('site/deletefileoldwpd'); ?>",
						data: data,
	...
	function uploadfileoldwpdtosftp(tffs_id, tffs_name, rowno){
		...
		$.ajax({
			type: "POST", 
			url: "<?php echo Yii::app()->createAbsoluteUrl('site/calluploadfileoldwpdtosftp'); ?>",      
			data: data1,         
			success: function (da)
			{
...
<?php
	...
	$model = Textfileforsapiens2Tb::model()->findAll($qusg);
...
<table id="tffstb" class="display row-border responsive nowrap" style="width:100%; height:auto; color:#003;">
    ...
    <?php
		...
		foreach ($model as $rows){ 
		    ...
		    $link_path = Yii::app()->params['servicelocalpath'] . "/wpdtextfile/wpdold/" . $tffs_name;
	?>
    	<tr>
            ...
            <td style="text-align:center;">
            	...
            	<button class="btn btn-warning" title="เขียนข้อมูลลงเท็กซ์ไฟล์" onClick="javascript:writedatatotextfile(<?=$tffs_id?>,<?=$rowno?>,'<?=$tffs_name?>');"><i class="fa fa-edit"></i></button>
                <span></span>
                <?php }else{ ?>
            ...
            <td style="text-align:center;">
            	<?php if($tffs_status < 3){ ?>
            	<button id="btnupload<?=$rowno?>" class="btn btn-primary" title="อัปโหลดไฟล์ขึ้น SFTP Server" onClick="javascript:uploadfileoldwpdtosftp(<?=$tffs_id?>,'<?=$tffs_name?>',<?=$rowno?>);"><i class="fa fa-upload"></i></button>
            ...
            <td style="text-align:center;">
            	<?php if($tffs_status < 3){ ?>
            	<button class="btn btn-danger" title="ลบไฟล์" onClick="javascript:deletefileoldwpd(<?=$tffs_id?>,<?=$rowno?>,'<?=$tffs_name?>');"><i class="fa fa-trash"></i></button>
                <span></span>
                <?php } ?>
            </td>
        </tr>
...
?>
		<div style="color:#F00;"><i class="fa fa-exclamation"></i> ยังไม่มีรายการ textfile ในระบบ. </div>
<?php
	}//if countmodel > 0
?>
</body>
