		...
		if(d1){
			if(d2){
				if(mydate1<=mydate2){
					...
					data1 = "action=showrptetp&d1=" + d1 + "&d2=" + d2 + "&sso1=" + sso1 + "&bct=" + bct + "&bc=" + bc  + "&ssos2=" + ssos2 + "&sso1t=" + sso1t;
					
					//alert(data1);
					
					$.ajax({
						type: "POST", 
						url: "<?php echo Yii::app()->createAbsoluteUrl('site/callsubrptetp'); ?>",      
						data: data1,         
						success: function (da)
						{
						   //alert(da);
 ...
 <div id="rpttoolbar">
	<div class="thfont5">
        ...
        <div style="float:left; width:20%;">
        	<div style="float:left; width:100%;">
        		สถานะตรวจสอบ:
                <select id="ssos2" onChange="javascript:schdata1();" class="thfont3" style="border-radius:5px; padding-left:5px; padding-right:5px;">
                	<option value="0">ทุกสถานะ</option>
                    <option value="1">NE</option>
                    <option value="2">ZR</option>
                    <option value="3">CL</option>
					<option value="5">AL</option>
                    <option value="4">ไม่ตอบกลับ</option>
                </select>
            </div>
        </div><!--//style-->
        
        <div style="float:left; width:15%; text-align:left;">
        	<button onClick="javascript:schdata1();" class="thfont3" style="border-radius:5px; cursor:pointer;"><i class="fa fa-search" style="color:#666;"></i> ค้นหา</button>
        </div><!--//style-->
