...
<body>
	...
	<table id="sbranch1" cellpadding="5" class="table4_1 display row-border responsive nowrap" style="width:100%; height:auto; color:#003;">
      ...
      <?php
	  	$rowno = 1;
		$model = BranchTmpTb::model()->findAllByAttributes(array('registernumber'=>$regisnum, 'ordernumber'=>0));
		foreach ($model as $rows){
			...
			$email = $rows->email;
			if($email=='-'){
				$model2 = EmpstateTb::model()->findAllByAttributes(array('ems_registernumber'=>$regisnum));
				$cm2 = count($model2);
				if($cm2>0){
