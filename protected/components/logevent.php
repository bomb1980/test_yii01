...
class logevent extends CApplicationComponent{
	...
	public function sendmail($ema, $accno, $brnno, $cropname){ //use for production
	...
	public function sendmail3($ema, $accno, $brnno, $cropname){ //use for production
...
/*	public function sendmail($ema, $accno, $brnno, $cropname){ //use for localhost
		
		$txtsubject = "แจ้งผลการขี้นทะเบียนนายจ้างประกันสังคม";
		$txtbody = "สำนักงานประกันสังคม ได้กำหนดเลขที่บัญชีนายจ้างให้ท่านคือ {$accno} สถานประกอบการชื่อ  {$cropname} เมื่อท่านมีการจ้างลูกจ้าง ให้ท่านใช้เลขที่บัญชีนายจ้างตามที่แจ้ง ติดต่อขึ้นทะเบียนลูกจ้าง/ผู้ประกันตน ภายใน 30 วันนับจากวันที่เริ่มจ้างงาน ได้ที่ สปส. ณ ที่ตั้ง สำนักงานใหญ่ของท่าน สอบถามรายละเอียดได้ที่สายด่วน 1506 หากยังไม่ดำเนินกิจการหรือไม่มีลูกจ้าง ขอความร่วมมือท่านตอบแบบสำรวจอิเล็กทรอนิกส์ ตามข้อมูลจริง เนื่องจากข้อมูลดังกล่าวจะถูกนำมาใช้ในการวิเคราะห์เพื่อประโยชน์ต่อการคุ้มครองสิทธิที่พึงมีพีงได้ตามกฎหมาย และขอขอบคุณมา ณ โอกาสนี้ : CLICK เพื่อตอบแบบสำรวจ => (<a href='https://www.sso.go.th/etp/addressform.php?{$ema}&et=1'>ตอบแบบสอบถามอิเล็กทรอนิกส์</a>)";
		$txtsendform = "สำนักงานประกันสังคม";
		
		//echo "{$ema}, {$accno}, {$brnno}, {$cropname}";
		//exit;
		
		$mailer = new PHPMailer();
		$mailer->Mailer = "mail"; ////$mailer->IsSMTP();
		$mailer->IsSMTP();
		$mailer->SetLanguage('en');
		$mailer->Host = 'smtp.gmail.com'; //'smtp.gmail.com'; //'smtp.sso.go.th'
		$mailer->Port = 587; //587; //25
		$mailer->SMTPAuth = FALSE;
		$mailer->From = 'day.jakkrit@gmail.com'; //'day.jakkrit@gmail.com'; //'no-reply@sso.go.th';
		$mailer->FromName = 'sso_wpd'; 
		$mailer->IsHTML(true);
		$mailer->Body = 'test body';//iconv('utf-8','tis-620',$txtbody);
		$mailer->Subject = 'test subject'; //iconv('utf-8','tis-620',$txtsubject); 
		$mailer->AddAddress('day.jakkrit@gmail.com'); //$ema
...
/*public function sendmail3($ema, $accno, $brnno, $cropname){ //use for localhost
		
		$txtsubject = "แจ้งผลการขี้นทะเบียนนายจ้างประกันสังคม";
		$txtbody = "สำนักงานประกันสังคม ได้กำหนดเลขที่บัญชีนายจ้างให้ท่านคือ {$accno} สถานประกอบการชื่อ  {$cropname} เมื่อท่านมีการจ้างลูกจ้าง ให้ท่านใช้เลขที่บัญชีนายจ้างตามที่แจ้ง ติดต่อขึ้นทะเบียนลูกจ้าง/ผู้ประกันตน ภายใน 30 วันนับจากวันที่เริ่มจ้างงาน ได้ที่ สปส. ณ ที่ตั้ง สำนักงานใหญ่ของท่าน สอบถามรายละเอียดได้ที่สายด่วน 1506 หากยังไม่ดำเนินกิจการหรือไม่มีลูกจ้าง ขอความร่วมมือท่านตอบแบบสำรวจอิเล็กทรอนิกส์ ตามข้อมูลจริง เนื่องจากข้อมูลดังกล่าวจะถูกนำมาใช้ในการวิเคราะห์เพื่อประโยชน์ต่อการคุ้มครองสิทธิที่พึงมีพีงได้ตามกฎหมาย และขอขอบคุณมา ณ โอกาสนี้ : CLICK เพื่อตอบแบบสำรวจ => (<a href='https://www.sso.go.th/etp/addressform2.php?{$ema}&et=2'>ตอบแบบสอบถามอิเล็กทรอนิกส์</a>)";
		$txtsendform = "สำนักงานประกันสังคม";
		...
		$mailer->Password = "J@kO3122516";
		//$mailer->SetFrom("day.jakkrit@gmail.com", $txtsendform);
		
		$mailer->Mailer = "mail"; //IsSMTP()//"smtp";
		$mailer->SetLanguage('en');
		//$mailer->SMTPAuth = FALSE;
		$mailer->From = 'day.jakkrit@gmail.com'; //'day.jakkrit@gmail.com'; //'no-reply@sso.go.th';
		$mailer->FromName = $txtsendform; //'sso_wpd'; 
		$mailer->Body = $txtbody; //'test body';//iconv('utf-8','tis-620',$txtbody);
		$mailer->Subject = $txtsubject; //'test subject'; //iconv('utf-8','tis-620',$txtsubject); 
		$mailer->AddAddress($ema); //$ema //'day.jakkrit@gmail.com'
